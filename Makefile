...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-airflow-operator") | .version')
...
docker-build:
	docker build --force-rm --build-arg VERSION=${VERSION} -t "docker.stackable.tech/stackable/airflow-operator:${VERSION}" -f docker/Dockerfile .
...
version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/airflow-operator/Chart.yaml
...
crds:
	...
	cargo run --bin stackable-airflow-operator -- crd | yq eval '.metadata.annotations["helm.sh/resource-policy"]="keep"' - > deploy/helm/airflow-operator/crds/crds.yaml
