...
In this example, each Airflow worker in the `default` group will have a maximum of 2 gigabytes of memory. This memory limit applies to the container running Airflow but not to any other containers, such as the one used for collecting metrics.
...
In the example below, each Airflow worker in the `resources-from-role-group` role group will have a maximum of `2Gi` (gigabytes) of memory. The worker in role group `resources-from-role` does not have a specified memory/cpu limit and will therefore use the defaults (`1Gi` etc.) provided from the role.
...
For more details on how to specify resources in general for products running on the Stackable Data Platform see: xref:home:concepts:resources.adoc[Resource management].
...
=== LDAP
Airflow supports authentication of users against an LDAP server.
Have a look at https://github.com/stackabletech/airflow-operator/blob/main/examples/simple-airflow-cluster-ldap.yaml[the LDAP example] and the general xref:commons-operator::authenticationclass.adoc[Stackable Authentication] documentation on how to set it up.
In general, it requires you to specify a xref:commons-operator::authenticationclass.adoc[`AuthenticationClass`] which is used to authenticate the users. In the example below we assign all the users the `Admin` role once they log into Airflow:
...
=== LDAP
Airflow supports assigning roles to users based on their LDAP group membership
, though this is not yet supported by the Stackable operator. Currently, all the users logging in via LDAP get assigned the same role which you can configure via the attribute `authenticationConfig.userRegistrationRole` on the `AirflowCluster` object.
