...
Airflow can run in standalone mode (see ) but requires 2 external dependencies if we want to run jobs across more than one worker: these
dependencies are
...
the job metadata (in the example above we are using the same postgresl database for both).
...
- `spec.executor`: this setting determines how the cluster will run. For more information see (): the `CeleryExecutor`
...
- `workers`: the nodes over which the job workload will distributed by the scheduler
...
Click on an exampe DAG and then invoke the job: if the scheduler is correctly set up then the job
