...
The AuthenticationClass is then referenced in the AirflowCluster resource like so:
...
Users that log in with LDAP are assigned a default https://airflow.apache.org/docs/apache-airflow/stable/security/access-control.html#access-control[Role] upon logging in which is specified with the `userRegistrationRole` property.
...
All the users logging in via LDAP get assigned the same role which you can configure via the attribute `authenticationConfig.userRegistrationRole` on the `AirflowCluster` object:
