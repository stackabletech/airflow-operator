...
If this role is not specified then the process will be started as a scheduler subprocess (Airflow 2.x), or as a standalone process in the same container as the scheduler (Airflow 3.x+). We recommend specifying this role for production settings as running multiple processes in the same container complicates proper supervision.
...
* `spec.clusterConfig.credentialsSecretName`: specifies the secret containing the Airflow admin user information.
