...
<1> The name of the `Secret` used for accessing database and admin user details (included here to illustrate where different credential secrets are defined)
<2> a list of git-sync elements
<3> The name used to identify the git-sync definition (required)
<4> The repository that will be cloned (required)
<5> The branch name (defaults to `main`)
<6> The synchronisation interval in seconds (defaults to 10 seconds)
...
<8> A map of optional configuration settings that are listed in https://github.com/kubernetes/git-sync/tree/v3.6.4#primary-flags[this] configuration section (and the ones that follow on that link)
<9> The location of the DAG folder, relative to the repository path (required)
