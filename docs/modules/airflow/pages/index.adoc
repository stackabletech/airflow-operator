...
NOTE: Redis is only needed if `spec.executor` has been set to `celery` as the jobs will be queued via Redis before being assigned to a `worker` pod. When using the `kubernetes` executor the scheduler will take direct responsibility for this.
