...
The `worker` role is defined implicitly when `spec.executor` is set to `celery` but is not used when set to `kubernetes` as pods are created dynamically as needed. This means that the `worker` role structure - configuration, logging, resources etc. - is slightly different depending on the executor. This is illustrated below:
...
i.e. for the `kubernetes` executor there exists only a top-level config for the resources and logging settings (since there are no separate roles or role groups), plus a field for setting/overriding environment variables (this is necessary primarily for defining the DAGs folder).
...
NOTE: Redis is only needed if `spec.executor` has been set to `celeryExecutors` as the jobs will be queued via Redis before being assigned to a `worker` pod. When using `kubernetesExecutors` the scheduler will take direct responsibility for this.
