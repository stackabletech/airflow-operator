...
The classes `SparkKubernetesOperator` and `SparkKubernetesSensor` are in dedicated files as they will typically be used for all custom resources and thus are best defined separately from that the DAG that calls them. This also demonstrates that modularized DAG structures can be used for defining jobs as long as all dependencies exist in or below the root folder pulled by git-sync.
