...
NOTE: git-sync can be used with DAGs that make use of modularized files, as python will be configured internally to use the git-sync target folder as the "root" location when looking for referenced files. See the xref:usage-guide/applying-custom-resources.adoc[] example for more details.
