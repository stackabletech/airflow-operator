...
Kubernetes Executors and their respective Pods only live as long as the DAG they run. Afterwards the Pod is immediatly terminated and e.g. console output or logs are gone.

In order to persist executor logs, Airflow can be configured to store its https://airflow.apache.org/docs/apache-airflow-providers-cncf-kubernetes/stable/kubernetes_executor.html[executor logs on disk (PV)] 
or as described in the following section on S3.
