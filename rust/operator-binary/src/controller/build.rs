//! The build step in the AirflowCluster controller
//!
//! Takes a [`ValidatedAirflowCluster`] and produces [`KubernetesResources<Prepared>`].
//! All methods in this module are infallible — validation happens in the validate stage.

pub mod role_group_builder;

use std::marker::PhantomData;

use role_group_builder::RoleGroupBuilder;
...
use crate::{
    crd::{
        APP_NAME, AirflowExecutor, AirflowRole, Container, HTTP_PORT, HTTP_PORT_NAME, METRICS_PORT,
        METRICS_PORT_NAME, OPERATOR_NAME,
    },
    framework::{self, builder::meta::ownerreference_from_resource},
};
...
pub fn build(validated: &ValidatedAirflowCluster) -> KubernetesResources<Prepared> {
    ...
    // --- RBAC ---
    let rbac_labels = recommended_labels(validated, "rbac", "rbac");
...
fn build_headless_service(
    ...
    Service {
        ...
        spec: Some(ServiceSpec {
            ...
            selector: Some(
                role_group_selector_labels(cluster, &rolegroup_ref.role, &rolegroup_ref.role_group)
                    .into(),
            ),
...
fn recommended_labels(cluster: &ValidatedAirflowCluster, role: &str, role_group: &str) -> Labels {
    use crate::framework::types::operator::*;
...
fn build_group_listener(
    ...
    let metadata = ObjectMetaBuilder::new()
        ...
        .namespace(&cluster.namespace)
        .ownerreference(ownerreference_from_resource(cluster, None, Some(true)))
        .with_labels(recommended_labels(cluster, &role.to_string(), "none"))
        .build();
...
mod tests {
    ...
    use crate::{
        ...
        framework::{
            ...
            types::{kubernetes::Uid, operator::ClusterName},
        },
    };

    // Note: airflow_cluster() helper removed — build() no longer takes a raw CRD argument
    ...
    fn validated_cluster() -> ValidatedAirflowCluster {
        use crate::framework::types::kubernetes::NamespaceName;
