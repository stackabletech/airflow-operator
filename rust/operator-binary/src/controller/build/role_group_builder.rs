//! Builder for role group Kubernetes resources
//!
//! The [`RoleGroupBuilder`] constructs ConfigMaps and StatefulSets for a single role
//! group. All build methods are infallible — validation and config generation happen in
//! the validate stage.
...
impl<'a> RoleGroupBuilder<'a> {
    ...
    pub fn build_stateful_set(&self) -> StatefulSet {
        ...
        let match_labels = {
            ...
            crate::framework::kvp::label::role_group_selector(
                self.cluster,
    ...
    fn recommended_labels(&self) -> Labels {
        use crate::framework::types::operator::*;
        crate::framework::kvp::label::recommended_labels(
            self.cluster,
            &ProductName::from_str_unsafe(APP_NAME),
